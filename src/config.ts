...
export function loadConfig(opts: LoadConfigOpts): ResolvedConfig {
	...
	const defaultThreshold = (toml.default_axis_threshold as number) ?? DEFAULT_THRESHOLD;
	const rubric = (toml.rubric as Partial<Record<AxisName, number>>) ?? {};
	const thresholds = {} as Record<AxisName, 1 | 2 | 3 | 4 | 5>;
	for (const axis of AXIS_NAMES) {
		const v = (rubric[axis] ?? defaultThreshold) as 1 | 2 | 3 | 4 | 5;
		thresholds[axis] = v;
	}
...
function resolveRoleModel(
	...
	return {
		...
		thinking: (roleToml?.thinking as ThinkingLevel) ?? DEFAULT_THINKING[role],
	};
}
